...
tmp_test=$(kubectl get pods -Ao jsonpath='{range .items[*]}{@.status.containerStatuses[?(@.restartCount>0)].name}{" "}{end}' |wc -w)

if [ "$tmp_test" -gt "0" ]; then
echo "The following pods have restarted multiple times"  | tee -a "${LOGFILE}"
tmp_data=$(kubectl get pods -Ao jsonpath='{range .items[*]}{@.status.containerStatuses[?(@.restartCount>0)].name}{" "}{end}' | awk '{$1=$1};1')
